...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/card%filename%.pug
  )> newFile.pug
  type card.pug >> newFile.pug
  move /Y newFile.pug  card.pug
  (
...
(
echo @import '%fileName%/card%fileName%';
)>>card.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=card
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=card%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/card%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
